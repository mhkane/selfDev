...
class Classifier:
    ...
    def normalizeColumn(self, columnNumber):
       ...
       minimum = self.getMin(col)
       maximum = self.getMax(col)
       #print("Median: %f   ASD = %f" % (median, asd))
       self.medianAndDeviation.append((median, asd))
       self.minAndMax.append((minimum,maximum))
       for v in self.data:
    ...
    def normalizeVector(self, v):
        ...
        return vector
    def standardizeVector(self, v):
        """We have stored the median and asd for each column.
        We now use them to normalize vector v"""
        vector = list(v)
        for i in range(len(vector)):
            (minimum, maximum) = self.minAndMax[i]
            amp = maximum-minimum
            vector[i] = (vector[i] - minimum) / amp
        return vector
    ...
    def euclidean(self,vector1,vector2):
        ...
        return math.sqrt(sum(map(lambda v1,v2 : abs(v1-v2)*abs(v1-v2),vector1,vector2)))
    ...
    def nearestNeighbor(self, itemVector):
        ...
        return min([ (self.euclidean(itemVector, item[1]), item)
                     for item in self.data])
    
    def classify(self, itemVector):
        """Return class we think item Vector is in"""
        return(self.nearestNeighbor(self.standardizeVector(itemVector))[1][0])
