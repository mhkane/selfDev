...
class Classifier:
    ...
    def normalizeColumn(self, columnNumber):
       ...
       col = [self.data[i][1][columnNumber] for i in range(len(self.data))]
       med = self.getMedian(col)
       asd = self.getAbsoluteStandardDeviation(col,med)
       for j in range(len(self.data)):
        temp = (col[j]-med)/float(asd)   
        self.data[j][1][columnNumber]= temp
    def normalizeList(self, liste):
       """given a column number, normalize that column in self.data
       using the Modified Standard Score"""
       med = self.getMedian(liste)
       asd = self.getAbsoluteStandardDeviation(liste,med)
       for j in range(len(liste)):
        temp = (liste[j]-med)/float(asd)   
        liste[j]= temp
       return liste
...
def unitTest():
    ...
    print classifier.normalizeList([21,15,12,3,7])
    #
